...
def get_recommendation(request: RecommendationRequest) -> RecommendationResponse:
    ...
    # --- Tier 0: no topic yet at all — brand new learner, recommend a starting topic ---
    if not request.topic:
        starting_topic = STARTING_TOPIC_MAPPING.get(request.technicalExperience, "core_java")
        reason = (
            f"Based on your {request.technicalExperience} experience level, "
            f"we recommend starting with {starting_topic.replace('_', ' ').title()}."
        )
        return RecommendationResponse(
            userId=request.userId,
            topic=starting_topic,
            targetErrorCategory=None,
            recommendedChallengeType=f"{starting_topic.replace('_', ' ').title()} Starting Module",
            reason=reason,
            basis="new_learner_experience_level"
        )
